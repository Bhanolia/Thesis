...
class Update_Announcement extends StatefulWidget {
  final Announcement_Model announcement_model;

  Update_Announcement({required this.announcement_model});

  @override
  _Update_Announcement createState() => _Update_Announcement();
}

class _Update_Announcement extends State<Update_Announcement> {
                          ...
                          return InkWell(
                            ...
                            onTap: () {
                              ...
                              if (filesUrl.isNotEmpty &&
                                ...
                                // print(filesUrl);
                                DraftDeletedAnnouncement.add(filesUrl[index]);
  ...
  Future deleteimageupdated() async {
    setState(() {
      finalheaderurl = widget.announcement_model.Header;
      // previewtextvisibile = true;
      isheaderfilled = true;
      showheaderlocal = false;
      showheaderurl = true;
  ...
  Future synccategoryheader() async {
    if (widget.announcement_model.Categories == "Academic") {
      setState(() {
        _categories = 1;
      });
    } else {
    ...
    if (widget.announcement_model.Header != "") {
      setState(() {
        isheaderfilled = true;
        finalheaderurl = widget.announcement_model.Header;
        showimage = widget.announcement_model.Header;
      });
    } else {
      setState(() {
  ...
  Future sync_attachment() async {
    await Future.forEach(widget.announcement_model.Attachment, (element) async {
  ...
  Future sync_Controller() async {
    TittleControler.text = widget.announcement_model.Tittle;
    DescriptionControler.text = widget.announcement_model.Description;
  }
  ...
  Future Upload_Storage() async {
    ...
    try {
      ...
      await Future.forEach(sumfiles_local, (element) async {
        ...
        final Folder = widget.announcement_model.Id;
  ...
  Future deleteAtachmentonline() async {
    // print("List Url Will Deleted $DraftDeletedAnnouncement");
    try {
      await Future.forEach(DraftDeletedAnnouncement, (element) async {
      ...
      //       return Center(
      //           child: AlertDialog(
      //             content: Text("Delete Attachment Failed"),
      //           ));
      //     });
    }
  }
